...
void Model::Create(DeviceContext* pDevice, std::string_view Filepath)
{
	...
	m_IndexBuffer	= std::make_unique<IndexBuffer>(pDevice, BufferData(m_Indices.data(), m_Indices.size(), sizeof(uint32_t) * m_Indices.size(), sizeof(uint32_t)), BufferDesc());
...
void Model::Draw(Camera* pCamera)
{
	...
	for (size_t i = 0; i < m_Meshes.size(); i++)
	{
		...
		m_Device->GetCommandList()->SetGraphicsRoot32BitConstants(4, sizeof(indices) / 4, &indices, 0);
